...
import mas.cv4.onto.TransactionInfo;

/**
 * Created by Martin Pilat on 11.2.14.
 *
 * An implementation of the environment which takes care of the trading between
 * teh agents, send books and stores information about agents.
 */
public class Environment extends Agent {
	...
	private class HandleInfoBehaviour extends OneShotBehaviour {
		...
		public void action() {
			...
			// System.out.println("Got info request from " +
	...
	private class HandleSendBehaviour extends OneShotBehaviour {
		...
		public void action() {
			...
			if (!unfinishedTransaction.containsKey(transactionID)) { // this is
																		// the
																		// first
																		// time
																		// we
																		...
																		// about
																		// transaction
				unfinishedTransaction.put(transactionID,
						new TransactionInfo(sendMsgContent, sendMsg, System.currentTimeMillis()));
				return;
			}
			...
			for (int i = 0; i < sendOrder1.getSendingBooks().size(); i++) {
				if (sendOrder1.getSendingBooks().get(i).getBookID() != sendOrder2.getReceivingBooks().get(i)
						.getBookID()) {
					sendFailure(sendMsg1, sendMsg2, "orders do not match");
					return;
				}
			...
			for (int i = 0; i < sendOrder2.getSendingBooks().size(); i++) {
				if (sendOrder2.getSendingBooks().get(i).getBookID() != sendOrder1.getReceivingBooks().get(i)
						.getBookID()) {
					sendFailure(sendMsg1, sendMsg2, "orders do not match");
					return;
				}
			...
			send(reply1);
			send(reply2);
			System.out.println("Trade performed between agent " + agentName1 + " and " + agentName2 + ".");
			System.out.println(" - Agents exchanged " + sendOrder1.getSendingMoney() + " money and books: "
					+ sendOrder1.getSendingBooks().toString() + " for " + sendOrder2.getSendingMoney()
					+ " money and books: " + sendOrder2.getSendingBooks().toString() + ".");
		...
		// send FAILURE to both agent, the failure is described as text, only
		// performative is important
		void sendFailure(ACLMessage msg1, ACLMessage msg2, String text) {
			...
			send(reply1);
			send(reply2);
			System.out.println("Transaction failure: " + text);
		}
